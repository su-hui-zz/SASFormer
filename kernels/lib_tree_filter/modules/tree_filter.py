...
class MinimumSpanningTree(nn.Module):
    ...
    @staticmethod
    def _build_matrix_index(fm):  # [bz, 3, 128, 128]
    ...
    def _build_feature_weight(self, fm):  # [bz, 3, 128, 128]
        batch = fm.shape[0]
    ...
    def _build_label_weight(self, fm):   # [bz, 3, 128, 128]
        batch = fm.shape[0]
        weight_row = self.distance_func(fm[:, :, :-1, :], fm[:, :, 1:, :])
        weight_col = self.distance_func(fm[:, :, :, :-1], fm[:, :, :, 1:])
        weight_row = weight_row.reshape([batch, -1])
    ...
    def forward(self, guide_in, label=None):  # guide_in - [bz, 3, 128, 128]
        with torch.no_grad():
